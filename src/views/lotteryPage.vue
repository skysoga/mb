...
<script>
  ...
  var _fetchLive = function (data, option = {}){
    ...
    if(data.Action!=='GetServerTimeMillisecond'){
      ...
      for (var i = 0; i < fetchArrLive.length; i++) {
        if(fetchArrLive[i][0]+1000<now){
          fetchArrLive.length=i
          break
        }else if(fetchArrLive[i][1]===str){
