var Live = {
  ...
  CreateVideoElements: function() {
    ...
    for (var i = 0; i < elements.length; i++) {
      new Live.VideoElement(elements[i])
    }
  },
...
if (document.readyState === "complete") {
  Live.CreateVideoElements()
} else {
  document.addEventListener("DOMContentLoaded", Live.CreateVideoElements)
}
Live.VideoElement = function() {
  "use strict";
  var VideoElement = function(element) {
    ...
    this.player = new Live.Player(url, options);
...
Live.Player = function() {
  ...
  var Player = function(url, options) {
    ...
    if (options.source) {
      ...
      this.source = new Live.Source.WebSocket(url, options);
      options.streaming = true
    } else if (options.progressive !== false) {
      this.source = new Live.Source.AjaxProgressive(url, options);
      options.streaming = false
    } else {
      this.source = new Live.Source.Ajax(url, options);
      options.streaming = false
    }
    ...
    if (options.video !== false) {
      this.video = new Live.Decoder.MPEG1Video(options);
    ...
    if (options.audio !== false && Live.AudioOutput.WebAudio.IsSupported()) {
      this.audio = new Live.Decoder.MP2Audio(options);
  ...
  Player.prototype.update = function() {
    ...
    if (!this.isPlaying) {
      this.isPlaying = true;
      this.startTime = Live.Now() - this.currentTime
    }
  ...
  Player.prototype.updateForStaticFile = function() {
    ...
    if (this.audio && this.audio.canPlay) {
      ...
      var targetTime = Live.Now() - this.startTime + this.video.startTime,
...
Live.Source.AjaxProgressive = function() {
  "use strict";
  ...
  AjaxProgressiveSource.prototype.onChunkLoad = function(data) {
    ...
    this.loadTime = Live.Now() - this.loadStartTime;
    if (!this.throttled) {
      this.loadNextChunk()
    }
  };
...
Live.Demuxer.TS = function() {
  ...
  TS.prototype.write = function(buffer) {
    if (this.leftoverBytes) {
      ...
      this.bits = new Live.BitBuffer(buffer)
    }
  ...
  TS.prototype.resync = function() {
    ...
    console.warn("Live: Possible garbage data. Skipping.");
...
Live.Decoder.MPEG1Video = function() {
  ...
  var MPEG1 = function(options) {
    Live.Decoder.Base.call(this, options);
    var bufferSize = options.videoBufferSize || 512 * 1024;
    var bufferMode = options.streaming ? Live.BitBuffer.MODE.EVICT : Live.BitBuffer.MODE.EXPAND;
  ...
  MPEG1.prototype.write = function(pts, buffers) {
    Live.Decoder.Base.prototype.write.call(this, pts, buffers);
...
Live.Decoder.MP2Audio = function() {
  "use strict";
...
Live.Renderer.Canvas2D = function() {
  "use strict";
...
Live.AudioOutput.WebAudio = function() {
  "use strict";
  ...
  WebAudioOut.prototype.getEnqueuedTime = function() {
    return Math.max(this.wallclockStartTime - Live.Now(), 0)
  };
